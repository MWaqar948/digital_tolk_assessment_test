...
class NotificationService implements NotificationServiceInterface {
     ...
     */
    function __construct(MailerInterface $mailer, JobRepositoryInterface $jobRepository, BookingRepositoryInterface $bookingRepository )
    {
        $this->mailer = $mailer;
    ...
    {
        $translators = $this->bookingRepository->getPotentialTranslators($job);
        $jobPosterMeta = UserMeta::where('user_id', $job->user_id)->first();

        // prepare message templates
        $date = date('d.m.Y', strtotime($job->due));
        $time = date('H:i', strtotime($job->due));
        $duration = $this->bookingRepository->convertToHoursMins($job->duration);
        $jobId = $job->id;
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        $user_tags = $this->bookingRepository->getUserTagsStringFromArray($users);
